    ...
    pub fn execute(&mut self, inst: u64) -> Result<u64, Exception> {
        ...
        match opcode {
            ...
            0x13 => {
                ...
                // Shift amount encoded in lower 6 bits of I-immediate field for RV64I
            ...
            0x67 => {
                ...
                let t = self.pc + 4;
                let imm = ((((inst & 0xfff00000) as i32) as i64) >> 20) as u64;
                let npc = (self.regs[rs1].wrapping_add(imm)) & !1;
                self.regs[rd] = t;
                return Ok(npc);
            }
            0x6f => {
                    ...
                    0x0 => {
                        match (rs2, funct7) {
                            (0x2, 0x8) => {
                                ...
                                let npc = self.csr.load(SEPC) & !0b11;
                                return Ok(npc);
                            }
                            (0x2, 0x18) => {
                                // mret
                                ...
                                let npc = self.csr.load(MEPC) & !0b11;
                                return Ok(npc);
                            }
                            (_, 0x9) => {
                                return self.update_pc();
...
mod test {
    ...
    fn generate_rv_obj(assembly: &str) {
        ...
        let output = Command::new(cc)
            ...
            .arg(assembly)
            .output()
            .expect("Failed to generate rv objects");
        println!("{}", String::from_utf8_lossy(&output.stderr));
    }
